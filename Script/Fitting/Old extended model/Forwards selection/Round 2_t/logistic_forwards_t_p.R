...
capture.output(fit,
               file="~/rsv-modelling/Output/Data/Forwards_selection/Round 2_t/logistic_forwards_t_p.txt")

#calculating AIC
aic <- 2*length(initial)+2*neglk.parameters.logistic(fit$par, train.df, known, weather, include)

#saving
write.csv(aic, "~/rsv-modelling/Output/Data/Forwards_selection/Round 2_t/logistic_forwards_t_p.csv")