...
#the extended models here are the old versions that use six variables
library(deSolve)
library(tidyverse)
theme_set(theme_bw())
...
ode.solve.loglinear <- function(variable, #variable parameters
  ...
  betas = exp(c.0
             ...
             +cs[2]*weather$evaporation
             +cs[3]*weather$temperature
             +cs[4]*weather$dewpoint
             +cs[5]*weather$humidity
             +cs[6]*weather$school)
