...
capture.output(fit,
               file="~/rsv-modelling/Output/Data/Fits/Backwards selection/Fit_logistic_v_output.txt")
write.csv(fit$par,
          file="~/rsv-modelling/Output/Data/Fits/Backwards selection/Fit_logistic_v_par.csv")

#calculating AIC
aic <- 2*length(initial)+2*neglk.parameters.logistic(fit$par, train.df, known, weather, include)
write.csv(aic, "~/rsv-modelling/Output/Data/Fits/Backwards selection/Fit_logistic_v_aic.csv")
...
mse <- ssq.parameters.logistic(fit$par,
                                    ...
                                    weather=weather,
                                    include=include)
write.csv(mse, file="~/rsv-modelling/Output/Data/Fits/Backwards selection/Fit_logistic_v_mse.csv")