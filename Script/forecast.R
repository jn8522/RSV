#calculating each model's forecast from the end of the study period through to 2021
...
#Forecasts for different lockdown efficacies
forecast.loglinear100 <- ode.solve.loglinear(variable=par.loglinear,
                                          known=c(known, 100),
                                          noise=FALSE,
                                          weather=weather,
                                          include=include)%>%
  mutate(Efficacy="100%")
forecast.loglinear75 <- ode.solve.loglinear(variable=par.loglinear,
                                            known=c(known, 75),
                                            noise=FALSE,
                                            weather=weather,
                                            include=include)%>%
  mutate(Efficacy="75%")
forecast.loglinear50 <- ode.solve.loglinear(variable=par.loglinear,
                                            known=c(known, 50),
                                            noise=FALSE,
                                            weather=weather,
                                            include=include)%>%
...
forecast.loglinear=union(forecast.loglinear100,forecast.loglinear75)%>%
  union(forecast.loglinear50)%>%
  mutate(Efficacy=factor(Efficacy, levels=c("50%", "75%", "100%")))

forecast.base100 <- ode.solve(variable=par.base,
                           weeks=nrow(weather),
                           known=c(known, 100),
                           noise=FALSE)%>%
  mutate(Efficacy="100%")

forecast.base75 <- ode.solve(variable=par.base,
                              weeks=nrow(weather),
                              known=c(known, 75),
                              noise=FALSE)%>%
  mutate(Efficacy="75%")

forecast.base50 <- ode.solve(variable=par.base,
                              weeks=nrow(weather),
                              known=c(known, 50),
                              noise=FALSE)%>%
  mutate(Efficacy="50%")

forecast.base=union(forecast.base100,forecast.base75)%>%
  union(forecast.base50)%>%
  mutate(Efficacy=factor(Efficacy, levels=c("50%", "75%", "100%")))
...
loglinear.plot <- forecast.loglinear %>% 
  ...
  geom_vline(xintercept=1056/(365.25/7)+2000, linetype="dashed")+
  annotate(geom="text", x=(1056+5)/(365.25/7)+2000, y=60, label="Lockdown", size=3)+
  geom_vline(xintercept=1066/(365.25/7)+2000, linetype="dashed")+
  geom_vline(xintercept=(interval(dmy("1.1.2000"),dmy("1.6.2020"))%/% weeks(1)+1)/(365.25/7)+2000, linetype="dashed")+
  annotate(geom="text", x=(interval(dmy("1.1.2000"),dmy("1.6.2020"))%/% weeks(1)+6.5)/(365.25/7)+2000, y=60, label="Winter", size=3)+
  geom_vline(xintercept=(interval(dmy("1.1.2000"),dmy("1.9.2020"))%/% weeks(1)+1)/(365.25/7)+2000, linetype="dashed")+
  annotate(geom="text", x=(interval(dmy("1.1.2000"),dmy("1.9.2020"))%/% weeks(1)+6.5)/(365.25/7)+2000, y=60, label="Spring", size=3)+
  geom_vline(xintercept=(interval(dmy("1.1.2000"),dmy("1.12.2020"))%/% weeks(1)+1)/(365.25/7)+2000, linetype="dashed")+
  annotate(geom="text", x=(interval(dmy("1.1.2000"),dmy("1.12.2020"))%/% weeks(1)+6.5)/(365.25/7)+2000, y=60, label="Summer", size=3)+
  geom_vline(xintercept=(interval(dmy("1.1.2000"),dmy("1.3.2021"))%/% weeks(1)+1)/(365.25/7)+2000, linetype="dashed")+
  annotate(geom="text", x=(interval(dmy("1.1.2000"),dmy("1.3.2021"))%/% weeks(1)+6.5)/(365.25/7)+2000, y=60, label="Autumn", size=3)+
  geom_vline(xintercept=(interval(dmy("1.1.2000"),dmy("1.6.2021"))%/% weeks(1)+1)/(365.25/7)+2000, linetype="dashed")+
  annotate(geom="text", x=(interval(dmy("1.1.2000"),dmy("1.6.2021"))%/% weeks(1)+6.5)/(365.25/7)+2000, y=60, label="Winter", size=3)+
  labs(x="", y="Cases")+
